  ...
  return (
    <div className={`bg-luxury-black border-border overflow-hidden rounded-2xl border ${className}`}>
        ...
        {data.profileImage && data.name && (
          <div className="absolute right-4 bottom-4 left-4">
            <div className="mb-1 flex items-center gap-2">
              <h3 className="text-heading-lg font-bold text-white drop-shadow-lg">{data.name || "이름"}</h3>
              {data.gender && (
                <span className={`px-2 py-0.5 ${genderColor} text-caption rounded text-white`}>{data.gender}</span>
              )}
            </div>
      ...
      <div className="space-y-3 p-4">
        ...
        <div className="text-body-sm text-muted-gray flex flex-wrap gap-2">
        ...
        {data.skills.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {data.skills.slice(0, 5).map((skill) => (
              <span key={skill} className="bg-gold/10 text-gold text-caption rounded-full px-2 py-0.5">
                {skill}
              </span>
            ))}
            {data.skills.length > 5 && (
              <span className="bg-luxury-tertiary text-muted-gray text-caption rounded-full px-2 py-0.5">
                +{data.skills.length - 5}
              </span>
            )}
          </div>
